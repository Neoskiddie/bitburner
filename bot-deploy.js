// Very similar script to home-deploy.
// It tries to run the hack/weaken/grow ratio of 1:2:10 on all bots.
// What is meant by bots is the purchased servers with names like bot0, bot1 etc.
// It is good idea to run the servers with more threads as they gain more RAM
// With max RAM the game would crash if the scripts weren't run with 256 threads
/** @param {NS} ns */
export async function main(ns) {
	...
	let targets = getHackable(ns, allServers).filter(name => !name.includes("bot"));
	...
	for (let x = 0; x < bots.length; x++) {
		await runScriptsOnTheHackableServers(
			ns,
			deployDirectory,
			localDirectory,
			hackScript,
...
async function runScriptsOnTheHackableServers(
	...
	//ns.tprintf("runScriptsOnTheHackableServers called with following args" + targets + "\n" + threads);
	//+ deployDirectory + localDirectory + hackScript + growScript + weakenScript + attacker 
	let ram = ns.getServerMaxRam(attacker);

	const sumOfRatio = 13;
	const scriptRunningCost = 1.75 * threads;
	const numberOfTimesScriptCanBeRun = ram / scriptRunningCost;
	const maxRunningPerTarget = numberOfTimesScriptCanBeRun / targets.length;
	const valueOfOneShare = maxRunningPerTarget / sumOfRatio;
	//if (valueOfOneShare == 0) {
	//	ns.tprintf("Not enough ram on the bots to run with given number of threads, exiting.")
	//	return;
	//}
...
function getHackable(ns, listOfServers) {
	...
	for (let x = 0; x < listOfServers.length; x++) {
		...
		if (isHackable && hasRoot) {
			...
			ns.tprintf(host + " is hackable, but doesn't have root access!")
		}
	}
...
async function createScripts(ns) {
	ns.tprintf("create scripts called");
